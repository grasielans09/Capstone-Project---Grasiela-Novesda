...
# Fungsi Cari Siswa by id
def cari_siswa(key):
    return siswa_db.get(key, None)

# Menu Utama Siswa
def login_Siswa():
    idSiswa = input('\nMasukkan ID Siswa: ').strip()
    siswa = cari_siswa(idSiswa)

    if siswa:
        print(Fore.GREEN + f'\nLogin berhasil! Selamat datang, {siswa["namaSiswa"]}.' + Style.RESET_ALL)
        return idSiswa
    else:
...
def inputDataSiswa():
    ...
    namaSiswa = input('Nama Siswa: ').strip()
    kelas = print(f'''Kelas (JCDS100): {kelas}''')
    alamat = input('Alamat: ')

    key = auto_idSiswa(kelas)
...
def inputNilai():
    ...
    if key in siswa_db:
        if siswa_db[key]['tugas'] != 0 or siswa_db[key]['kuis'] != 0 or siswa_db[key]['uts'] != 0 or siswa_db[key]['uas'] != 0:
            ...
            print(Fore.RED + '\t\tSilakan gunakan menu "Edit Nilai" untuk mengubah nilai.' + Style.RESET_ALL).strip()
        else:
            while True:
                try:
                    tugas = int(input('Nilai Tugas: ')).strip()
                    kuis = int(input('Nilai Kuis: ')).strip()
                    uts = int(input('Nilai UTS: ')).strip()
                    uas = int(input('Nilai UAS: ')).strip()
